...
<kendo-grid
  ...
  [pageSize]="store['gridState']().take ?? 10"
...
>
  ...
  <ng-template kendoGridToolbarTemplate>
    <button kendoButton [svgIcon]="createIcon" (click)="createUser()" aria-label="Add"></button>
  </ng-template>

  <!-- Data Columns -->
  <kendo-grid-column field="UserId" title="ID" [width]="80"></kendo-grid-column>
  ...
  <kendo-grid-command-column title="Commands" [width]="160">
    <ng-template kendoGridCellTemplate let-dataItem>
      <button kendoButton [svgIcon]="updateIcon" [primary]="true" (click)="updateUser(dataItem.UserId)" aria-label="Edit">
      </button>
      <button kendoButton [svgIcon]="removeIcon" (click)="removeUser(dataItem)" aria-label="Remove"></button>
    </ng-template>
...
@if (itemToRemove) {
    <kendo-dialog title="Please confirm" (close)="confirmRemove(false)">
      <p style="margin: 30px; text-align: center;">
        Are you sure you want to delete user {{ itemToRemove.Email }}?
      </p>
...
export class UsersGrid {
  ...
  public itemToRemove: User | null = null;
  ...
  public removeUser(dataItem: User): void {
    this.itemToRemove = dataItem;
  }

  public confirmRemove(shouldRemove: boolean): void {
    if (shouldRemove && this.itemToRemove) {
      const userId = this.itemToRemove.UserId;
      console.log('🔵 confirmRemove: User CONFIRMED removal for user', userId);
      
      this.store['delete'](this.itemToRemove);
      
      if (userId && userId.length > 0) {
        console.log('🔵 confirmRemove: Calling removeFromServer', userId);
        this.store['removeFromServer'](userId);
      } else {
  ...
  public removeConfirmation(dataItem: User): Subject<boolean> {
    this.itemToRemove = dataItem;
